/* =========================================================
   ETB BACKEND — QUOTE ENGINE
   Il backend è l'unica autorità sul calcolo economico.
   ========================================================= */
...
function getFeePerMusician(distanceKm) {
  const band = ETB_QUOTE_CONFIG.feeBands.find(
    (item) => distanceKm >= item.minKm && distanceKm <= item.maxKm,
  );
...
function calculateFuelCost(distanceKmRoundTrip, kmPerLiter) {
  return (distanceKmRoundTrip / kmPerLiter) *
    ETB_QUOTE_CONFIG.fuelPricePerLiter;
}
...
export function calculateEtbQuote({
  eventDate,
  distanceKmOneWay,
  lineup = "full",
  ...
  const technicalDetails = (extra = {}) => ({
    ...
    tollCalculationMethod:
      "Stima prudenziale: distanza sola andata × costo stimato €/km",
