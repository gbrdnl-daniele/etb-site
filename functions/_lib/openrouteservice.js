/* =========================================================
   ETB BACKEND — OPENROUTESERVICE / HEIGIT
...
export async function searchPlaces(apiKey, query) {
  const url = new URL(`${ORS_GEOCODING_URL}/autocomplete`);
...
export async function geocodePlace(apiKey, placeLabel) {
  const url = new URL(`${ORS_GEOCODING_URL}/search`);
...
export async function calculateRoute(apiKey, destination) {
  const response = await fetch(
    `${ORS_DIRECTIONS_URL}/v2/directions/driving-car`,
    {
      method: "POST",
